...
def perform_SVD(documents, query, relevant, irrelevant, coffee_name_to_index):
    ...
    td_matrix = vectorizer.fit_transform(
        [x[-2] for x in documents]
    )  # combined description is last column in df
    ...
    query_vec = (
        normalize(np.dot(query_tfidf_new.reshape(1, -1), words_compressed)).squeeze().T
    )
